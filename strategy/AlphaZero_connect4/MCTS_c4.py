...
class UCTNode():
    ...
    def best_child(self):
        if self.action_index:
            ...
            bestmove = self.action_index[np.argmax(bestmove[self.action_index])]
        else:
    ...
    def add_dirichlet_noise(self, action_index, child_priors):
        valid_child_priors = child_priors[action_index]  # select only legal moves entries in child_priors array
        valid_child_priors = 0.75 * valid_child_priors + 0.25 * np.random.dirichlet(np.zeros([len(valid_child_priors)], \
                                                                                             dtype=np.float32) + 192)
        child_priors[action_index] = valid_child_priors
        return child_priors

    def expand(self, child_priors):
        self.is_expanded = True
        action_index = self.game.actions()
        c_p = child_priors
        if not action_index:
            self.is_expanded = False
        self.action_index = action_index
        c_p[[i for i in range(len(child_priors)) if i not in action_index]] = 0.000000000  # mask all illegal actions
        if self.parent.parent is None:  # add dirichlet noise to child_priors in root node
            c_p = self.add_dirichlet_noise(action_index, c_p)
